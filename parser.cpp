...
{    
        ...
        *currentStatement = *buf;
        ...
        if (temp.at(0)=="else")
        {
            ...
            *currentStatement = *buf;
            ...
            currentStatement->createNodeAbove(new IfOp(&stack->last(), stack, activeBlock, &Block::runChildBlock, ifTrue, ifFalse));
        ...
        else
        {
            currentStatement->setCurrentNodeToRoot();
            stack->push(MyVariant());
            currentStatement->createNodeAbove(new IfOp(&stack->last(), stack, activeBlock, &Block::runChildBlock, ifTrue));
            activeBlock->addStatement(currentStatement);
            delete currentStatement;
    ...
    case 3: //loop
    {
        ...
        *currentStatement = *buf;
        ...
        currentStatement->setCurrentNodeToRoot();
        stack->push(MyVariant());
        currentStatement->createNodeAbove(new LoopOp(&stack->last(), stack, activeBlock, &Block::runChildBlock, ifTrue));
        activeBlock->addStatement(currentStatement);
        delete currentStatement;
    ...
    case 5:
    {
        ...
        *currentStatement = *buf;
        ...
        currentStatement->setCurrentNodeToRoot();
        stack->push(MyVariant());
        currentStatement->createNodeAbove(new WhileOp(&stack->last(), stack, activeBlock, &Block::runChildBlock, ifTrue));
        activeBlock->addStatement(currentStatement);
        delete currentStatement;
...
bool Parser::boolExpression()
{
    ...
    while (!sp.lookIs('\0') && !sp.lookIs(')') && !sp.lookIs(',') && !sp.lookIs(';') && !sp.lookIs('}') && !sp.lookIs(']') && !sp.lookIs('='))
    {
        if (sp.lookIs('|')) {
            sp.match('|');
            stack->push(MyVariant());
            currentStatement->createNodeAbove(new BoolOrOp(&stack->last(), stack));
        }

        else if (sp.lookIs('~')) {
            stack->push(MyVariant());
            currentStatement->createNodeAbove(new BoolXorOp(&stack->last(), stack));
        }
        else {
            reportExpected("'|' or '~'");
        }
...
bool Parser::boolTerm()
{
    ...
    while (sp.lookIs('&'))
    {
        sp.match('&');
        stack->push(MyVariant());
        currentStatement->createNodeAbove(new BoolAndOp(&stack->last(), stack));
        currentStatement->goUp();
        if (!notFactor()) return false;
        currentStatement->goUp();
    }
...
bool Parser::notFactor()
{
    if (sp.lookIs('!'))
    {
        sp.match('!');
        stack->push(MyVariant());
        currentStatement->createNodeAbove(new BoolNotOp(&stack->last(), stack));
    }
...
bool Parser::relation()
{
    ...
    if (sp.lookIsRelop() && !sp.lookIsAssignmentOp())
    {
        if (sp.lookIs('=') && sp.nextIs('=')) {
            ...
            sp.match('=');
            stack->push(MyVariant());
            currentStatement->createNodeAbove(new IsEqualOp(&stack->last(), stack));
        }
            ...
            sp.match('=');
            stack->push(MyVariant());
            currentStatement->createNodeAbove(new IsNotEqualOp(&stack->last(), stack));
        }
            ...
            if (sp.lookIs('=')) {
                sp.match('=');
                stack->push(MyVariant());
                currentStatement->createNodeAbove(new IsLessEqualOp(&stack->last(), stack));
            }
            else {
                stack->push(MyVariant());
                currentStatement->createNodeAbove(new IsLessOp(&stack->last(), stack));
            }
        }
            ...
            if (sp.lookIs('=')) {
                sp.match('=');
                stack->push(MyVariant());
                currentStatement->createNodeAbove(new IsMoreEqualOp(&stack->last(), stack));
            }
            else {
                stack->push(MyVariant());
                currentStatement->createNodeAbove(new IsMoreOp(&stack->last(), stack));
            }
        }
...
bool Parser::add()
{
    sp.match('+');
    stack->push(MyVariant());
    currentStatement->createNodeAbove(new AddOp(&stack->last(), stack));
    return true;
}
...
bool Parser::substract()
{
    sp.match('-');
    stack->push(MyVariant());
    currentStatement->createNodeAbove(new SubstractOp(&stack->last(), stack));
    return true;
}
...
bool Parser::multiply()
{
    if (!sp.match('*')) return false;
    stack->push(MyVariant());
    currentStatement->createNodeAbove(new MultiplyOp(&stack->last(), stack));
    return true;
}
...
bool Parser::divide()
{
    if (!sp.match('/')) return false;
    stack->push(MyVariant());
    currentStatement->createNodeAbove(new DivideOp(&stack->last(), stack));
    return true;
}
...
bool Parser::powerFactor()
{
    ...
    while (sp.lookIs('^')) {
        sp.match('^');
        stack->push(MyVariant());
        currentStatement->createNodeAbove(new PowerOp(&stack->last(), stack));
        currentStatement->goUp();
        if (!factor()) return false;
        currentStatement->goUp();
    }
...
bool Parser::signedFactor()
{
    if (sp.lookIs('-')) {
            sp.match('-');
            stack->push(MyVariant());
            currentStatement->createRightChild(new UnaryMinus(&stack->last(), stack));
    }
    if (!powerFactor()) return false;
    return true;
}
...
bool Parser::factor()
{
    ...
    if (sp.lookIs('{'))
    {
        sp.match('{');
        stack->push(MyVariant());
        currentStatement->createRightChild(new InitializerList(&stack->last(), stack));
        while (!sp.lookIs('}'))
        {
            Statement* buf = new Statement;
            *buf = *currentStatement;
...
bool Parser::ident()
{
    ...
    if (presetFunctions.contains(name)) {
        ...
        case 0: //abs
            stack->push(MyVariant());
            currentStatement->createRightChild(new AbsOp(&stack->last(), stack));
            break;
        case 1: //log
            stack->push(MyVariant());
